...
/*
  * extract_interop: example interoperability test of a keypoint detector (in this case: SIFT).
  * extract SIFT descriptors using the same software architecture of the CDVS Library.
  */
#include <cassert>
#include <string>
#include <iostream>
...
{
    cout <<
      "CDVS descriptor extraction module (alternate detector implementation for interoperability test - do not use in production).\n"
      "Usage:\n"
	  "  extract_interop <images> <mode> <dataset path> <annotation path> [-p parameters] [-help]\n"
      "where:\n"
...
/**
 * @class SiftClient
 * Main SIFT-based implementation of the high level interface to the client-side functionality of the CDVS Library.
 * @author  Massimo Balestri
 * @date 2014
 */
class SiftClient  : public CdvsClient {
protected:
...
public:
	SiftClient(const CdvsConfiguration * config, int mode) {
		if (config == NULL)
	...
	virtual unsigned int encode(CdvsDescriptor & cdvsDescriptor, int width, int height, const unsigned char * buffer) const
	{
		cdvsDescriptor.clear();		// erase old data

		SiftDetector imagebuffer;
...
int run_extract(int argc, char *argv[])
{
	...
	CdvsConfiguration * cdvsconfig = CdvsConfiguration::cdvsConfigurationFactory(paramfile);	// replace some params
	CdvsClient * cdvsclient = new SiftClient(cdvsconfig, mode);		// get my SIFT client instance
...
// end of extract_interop example
