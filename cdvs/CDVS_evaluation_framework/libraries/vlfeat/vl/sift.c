...
// Added by ETRI : Defined to avoid repetitive calculation
#define N_PI2 (36.0 / VL_PI2)
#define PI2_N (1.0/N_PI2)
#define NBP_2 (NBP/2)
#define NBO_PI2 (NBO/VL_PI2)
#define DESCR_SIZE (NBO*NBP*NBP)
 ...
 **
//
// replaced by static declaration
//
VL_INLINE void
fast_expn_init ()
{
...
_vl_sift_smooth (VlSiftFilt * self,
                 ...
                 double sigma)
{
  /* prepare Gaussian filter - all computations performed at double precision */
  if (self->gaussFilterSigma != sigma) {
    ...
    ntaps = 2 * self->gaussFilterWidth + 1;
    dfilter = vl_malloc(sizeof(double) * ntaps);
    self->gaussFilter = vl_malloc (sizeof(vl_sift_pix) * ntaps) ;

    for (j = 0 ; j < ntaps ; ++j) {
      double d = ((signed)j - (signed)self->gaussFilterWidth) / sigma ;
      dfilter[j] =  exp (- 0.5 * (d*d)) ;
      acc += dfilter[j] ;
    }
    for (j = 0 ; j < ntaps ; ++j) {
      self->gaussFilter[j] = (vl_sift_pix) (dfilter[j] / acc) ;
    }
    vl_free(dfilter);
  }
...
/*
 * function added by TI: definitions for the ALP detector.
 * Just the interface changes, not the internal logic of vl_sift_calc_keypoint_orientations()
 */
VL_EXPORT
int
alp_keypoint_orientations (double angles [4], int w, int h, double sigma,
 double x, double y, const float * grad_mod, const float * grad_ang, double alp_magnif)
{
  double const winf   = alp_magnif / 2.0 ;
...
//  int const    xo     = 1 ;         /* x-stride */
  ...
  const float * pt_mod = grad_mod + xi + yo*yi;
  const float * pt_ang = grad_ang + xi + yo*yi;
  ...
  /* compute orientation histogram */
	...
	/* ignore first and last line and column (their values are not valid) */

	ymin = VL_MAX (-W, 1 - yi);
	ymax = VL_MIN (W, h - 2 - yi);
	xmin = VL_MAX (-W, 1 - xi);
	xmax = VL_MIN (W, w - 2 - xi);
...
/*
 * function added by ETRI: Input parameter float *src was added to alp_keypoint_orientations() for faster gradient computation
 * grad_mod and grad_ang is partially updated within this function
 */
VL_EXPORT
int
alp_keypoint_orientations_fast_mode(double angles [4], int w, int h, double sigma,
double x, double y, float * grad_mod, float * grad_ang, double alp_magnif, float * src)
{
	double const winf   = alp_magnif / 2.0 ;
	...
	float * pt_mod = grad_mod + xi + yo*yi; // Keundong Lee, Gradient buffers will be updated if necessary
	float * pt_ang = grad_ang + xi + yo*yi;

	// variables for gradient update (on-the-fly gradient computation)
	float *cp;
	double gx,gy; 
	float lp,rp,up,bp; 
	double exp_denom = 2*sigmaw*sigmaw;
	double W_2 = (double)(W*W) + 0.6;
	...
	for(ys = ymin; ys <= ymax; ++ys) {
		for(xs  = xmin; xs <= xmax ; ++xs) {
			...
			ang  = *(pt_ang + xs + ys*yo) ; // read the value of gradient angle in the map
			
			if(ang == 0.0){ // directly compute gradient (gradTheta can't have zero if it was calculated already)
				cp = src + (xi+ xs) + yo*(yi+ ys);
				lp = cp[-1]; // left
				rp = cp[+1]; // right
				up = cp[-w]; // upper
				bp = cp[+w]; // bottom
				...
				ang  = vl_mod_2pi_f   (vl_fast_atan2_f (gy, gx) + VL_PI2) ;

				*(pt_mod + xs + ys*yo) = mod; // update the map
				*(pt_ang + xs + ys*yo) = ang;

			}
			else{ // read the value (previously calculated)
				mod  = *(pt_mod + xs + ys*yo) ;
				ang  = *(pt_ang + xs + ys*yo) ;
			}

			wgt  = fast_expn (r2 / exp_denom) ; 
			fbin = N_PI2 * ang; 
...
/*
 * function added by TI: definitions for the ALP detector.
 * Just the interface changes, not the internal logic of vl_sift_calc_descriptor()
 */
VL_EXPORT
void
alp_keypoint_descriptor(float * descr, double x, double y, double sigma, double angle0, int w, int h, const float * gradMod, const float * gradTheta, double alp_magnif)
{
  // define constants used here
  double const magnification = alp_magnif;       /* magnification factor. */
  double const windowSize = 2;  		 /* size of Gaussian window (in spatial bins) */
  ...
  const float *pt_mod  = gradMod + xi*xo + yi*yo ;
  const float *pt_ang	= gradTheta + xi*xo + yi*yo ;
  ...
  for(dyi =  VL_MAX (- W, 1 - yi    ) ;
    ...
    for(dxi =  VL_MAX (- W, 1 - xi    ) ;
      ...
      /* retrieve */
      vl_sift_pix mod   = *( pt_mod + dxi*xo + dyi*yo) ;
      vl_sift_pix angle = *( pt_ang + dxi*xo + dyi*yo) ;
      vl_sift_pix theta = vl_mod_2pi_f (angle - angle0) ;
       ...
       * are in the normalized frame, so that -NBP/2 <= dx <=
       * NBP/2. */
      vl_sift_pix const wsigma = windowSize ;
      vl_sift_pix win = fast_expn
        ((nx*nx + ny*ny)/(2.0 * wsigma * wsigma)) ;
	...
	// precomputed products to avoid FP multiplications
	vl_sift_pix win_by_mod = win * mod;
	vl_sift_pix win_by_mod_by_rbinx, win_by_mod_by_rbinx_by_rbiny;
	vl_sift_pix weight;

    /* Distribute the current sample into the 8 adjacent bins*/

	//0-level loop for dbinx = 0
    if (binx  >= - (NBP/2) && binx  <    (NBP/2)) {

		win_by_mod_by_rbinx = win_by_mod * vl_abs_f (1 - rbinx);

		// 1-level loop for dbiny = 0
        if (biny  >= - (NBP/2) && biny  <    (NBP/2) ) {

          win_by_mod_by_rbinx_by_rbiny = win_by_mod_by_rbinx * vl_abs_f (1 - rbiny);

		  // 2-level loop for dbint = 0
		  weight = win_by_mod_by_rbinx_by_rbiny * vl_abs_f (1 - rbint) ;

          atd(binx, biny, (bint) % NBO) += weight ;

		  // 2-level loop for dbint = 1		      
          weight = win_by_mod_by_rbinx_by_rbiny * vl_abs_f (- rbint) ;

          atd(binx, biny, (bint+1) % NBO) += weight ;
        }

		// 1-level loop for dbiny = 1
        if (biny + 1 >= - (NBP/2) && biny + 1 <    (NBP/2) ) {

          win_by_mod_by_rbinx_by_rbiny = win_by_mod_by_rbinx * vl_abs_f (- rbiny);

		  // 2-level loop for dbint = 0
          weight = win_by_mod_by_rbinx_by_rbiny * vl_abs_f (1 - rbint) ;

          atd(binx, biny+1, (bint) % NBO) += weight ;

		  // 2-level loop for dbint = 1		        
          weight = win_by_mod_by_rbinx_by_rbiny * vl_abs_f (- rbint) ;

          atd(binx, biny+1, (bint+1) % NBO) += weight ;
        }
	}

	//0-level loop for dbinx = 1
    if (binx + 1 >= - (NBP/2) && binx + 1 <    (NBP/2)) {

		win_by_mod_by_rbinx = win_by_mod * vl_abs_f (- rbinx);
        ...
        if (biny  >= - (NBP/2) && biny  <    (NBP/2) ) {
          ...
          atd(binx+1, biny, (bint) % NBO) += weight ;

		  // 2-level loop for dbint = 1		        
          weight = win_by_mod_by_rbinx_by_rbiny * vl_abs_f (- rbint) ;

          atd(binx+1, biny, (bint+1) % NBO) += weight ;
        }
        ...
        if (biny + 1 >= - (NBP/2) && biny + 1 <    (NBP/2) ) {
          ...
          atd(binx+1, biny+1, (bint) % NBO) += weight ;

		  // 2-level loop for dbint = 1		        
          weight = win_by_mod_by_rbinx_by_rbiny * vl_abs_f (- rbint) ;

          atd(binx+1, biny+1, (bint+1) % NBO) += weight ;
        }
      }
    }
  }
    ...
    /* Normalize the histogram to L2 unit length. */
    norm = normalize_histogram (descr, descr + NBO*NBP*NBP) ;
...
/*
 * function added by ETRI: Input parameter float *src was added to alp_keypoint_descriptor() for faster gradient computation
 * grad_mod and grad_ang is partially updated within this function
 */
VL_EXPORT
void
alp_keypoint_descriptor_fast_mode(float * descr, double x, double y, double sigma, double angle0, int w, int h, float * gradMod, float * gradTheta, double alp_magnif,float* src) 
{
  // define constants used here
  double const magnification = alp_magnif;       /* magnification factor. */
  double const windowSize = 2;  		 /* size of Gaussian window (in spatial bins) */
  ...
  double const SBP    = magnification * sigma + VL_EPSILON_D ;
  int    const W      = floor
	  (1.4142135623730951 * SBP * (NBP + 1) / 2.0 + 0.5) ; 
  double W2 = (double)(W*W) + 0.6;  
  double const st0    = sin (angle0)/ SBP; 
  double const ct0    = cos (angle0)/ SBP; 
  vl_sift_pix const wsigma = windowSize ; 
  double exp_denom = (2.0 * wsigma * wsigma); 
  ...
  float *pt_mod  = gradMod + xi*xo + yi*yo ; // Gradient buffers will be updated if necessary
  float *pt_ang	= gradTheta + xi*xo + yi*yo ;
  ...
  /* clear descriptor */
  memset (descr, 0, sizeof(vl_sift_pix) * DESCR_SIZE) ; 
  

  /* Center the scale space and the descriptor on the current keypoint.
   * Note that dpt is pointing to the bin of center (SBP/2,SBP/2,0).
   */
  dpt = descr + (NBP_2) * binyo + (NBP_2) * binxo ; 
  ...
  for(dyi =  VL_MAX (- W, 1 - yi    ) ;
	...
	/* fractional displacement */
	 vl_sift_pix dx = xi + dxi - x;
	 vl_sift_pix dy = yi + dyi - y;
	 vl_sift_pix nx,ny,nt; 

	 double r2 = dx*dx + dy*dy ; 
	 if (r2 >= W2 + 0.6) continue ;

	 nx = ( ct0 * dx + st0 * dy)  ; 
	 ny = (-st0 * dx + ct0 * dy)  ; 

	 if(nx >= -2.5 && nx < 2.5 && ny>=-2.5 && ny<2.5)  // Early Rejection of irrelevant pixels which do not belong to descriptor patch
	 {
		 int binx,biny,bint;  
		 int bint0, bint1; 
		 float rbinx, rbiny, rbint;
		 float w1,w0,w01,w00,w11,w10,w001,w000,w011,w010,w101,w100,w111,w110,mm; // For fast tri-linear interpolation
       ...
       * NBP/2. */
		 vl_sift_pix win = fast_expn
			 ((nx*nx + ny*ny)/(exp_denom)) ; 
		 
		 vl_sift_pix angle = *( pt_ang + dxi*xo + dyi*yo) ;	 
		 vl_sift_pix mod, theta;

		 if(angle == 0.0){ // directly compute gradient (gradTheta can't have zero if it was calculated already)
			 float *cp;
			 double gx,gy; 
			 float lp,rp,up,bp; 

			 cp = src + (xi+dxi)*xo + (yi+dyi)*yo;
			 lp = cp[-1]; // left
			 rp = cp[+1]; // right
			 up = cp[-w]; // upper
			 bp = cp[+w]; // bottom
			 ...
			 mod  = vl_fast_sqrt_f(gx*gx+gy*gy);
			 angle  = vl_mod_2pi_f   (vl_fast_atan2_f (gy, gx) + VL_PI2) ;

			 *(pt_mod + dxi*xo + dyi*yo) = mod; // update the map
			 *(pt_ang + dxi*xo + dyi*yo) = angle;

		 }
		 else{
			 mod  = *(pt_mod + dxi*xo + dyi*yo) ; // read the value (previously calculated)
			 angle  = *(pt_ang + dxi*xo + dyi*yo) ;
		 }

		 theta = vl_mod_2pi_f (angle - angle0);
		...
		/* The sample will be distributed in 8 adjacent bins.
			We start from the ``lower-left'' bin. */
		binx = vl_floor_f (nx - 0.5) ;
		biny = vl_floor_f (ny - 0.5) ;
		bint = vl_floor_f (nt) ;
		rbinx = nx - (binx + 0.5) ;
		rbiny = ny - (biny + 0.5) ;
		rbint = nt - bint ;
		...
		if(binx >= -(NBP_2) && binx < (NBP_2)) {
			if(biny >= -(NBP_2) && biny < (NBP_2)){

				w001=w00*rbint;
				w000=w00-w001;

				atd(binx, biny, (bint0)) += w000;
				atd(binx, biny, (bint1)) += w001;

			}
			if((biny+1) >= -(NBP_2) && (biny+1) < (NBP_2)){

				w011=w01*rbint;
				w010=w01-w011;

				atd(binx, biny+1, (bint1)) += w011;
				atd(binx, biny+1, (bint0)) += w010;

			}

		}
		if((binx+1) >= -(NBP_2) && (binx+1) < (NBP_2)) {
			if(biny >= -(NBP_2) && biny < (NBP_2)){

				w101=w10*rbint;
				w100=w10-w101;

				atd(binx+1, biny, (bint1)) += w101;
				atd(binx+1, biny, (bint0)) += w100;

			}
			if((biny+1) >= -(NBP_2) && (biny+1) < (NBP_2)){
				w111 = w11*rbint;
				w110 = w11-w111;
				atd(binx+1, biny+1, (bint1)) += w111;
				atd(binx+1, biny+1, (bint0)) += w110;
    ...
    /* Normalize the histogram to L2 unit length. */
    norm = normalize_histogram (descr, descr + DESCR_SIZE) ; 

	/* Truncate at 0.2. */
	for(bin = 0; bin < DESCR_SIZE ; ++ bin) { 
		if (descr [bin] > 0.2) descr [bin] = 0.2;
	}

	/* Normalize again. */
	normalize_histogram (descr, descr + DESCR_SIZE) ; 

}

/*
 * function added by TI: definitions for the ALP detector.
 * Just the interface changes, not the internal logic of vl_smooth()
 */
VL_EXPORT
void alp_smooth (
                 float * outputImage,
...
/*
 * function added by TI: definitions for the ALP detector.
 * Just the interface changes, not the internal logic of update_gradient()
 */
VL_EXPORT
void
alp_update_gradient (float * gradmod, float *gradtetha, float * src, int w, int h)
{
 int const xo    = 1 ;
 int const yo    = w ;
 int y;

   vl_sift_pix *end, *grad, gx, gy ;

#undef SAVE_BACK
#define SAVE_BACK                                                       \
   *gradmod++ = vl_fast_sqrt_f (gx*gx + gy*gy) ;                          \
   *gradtetha++ = vl_mod_2pi_f   (vl_fast_atan2_f (gy, gx) + 2*VL_PI) ;     \
   ++src ;                                                             \
