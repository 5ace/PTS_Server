...
// Added by ETRI : defined to avoid repetitive computation
#define VL_PI2 (2.0*VL_PI)
#define VL_PI_1q (VL_PI/4.0)
#define VL_PI_3q (3.0*VL_PI_1q)
...
{
  while (x > (float)(VL_PI2)) x -= (float) (VL_PI2) ; // Keundong Lee
...
{
  while (x > VL_PI2) x -= VL_PI2 ;
  while (x < 0.0) x += VL_PI2 ;
  return x ;
}
...
vl_fast_atan2_f (float y, float x)
{
  ...
  if (x >= 0) {
    r = (x - abs_y) / (x + abs_y) ;
    angle = (float) (VL_PI_1q) ; // Keudnong Lee
  } else {
    r = (x + abs_y) / (abs_y - x) ;
    angle = (float) (VL_PI_3q) ;
  }
  angle += (c3*r*r - c1) * r ;
