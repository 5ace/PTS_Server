...
#include "AlpDetectorLowMem.h" 	// fast implementation of the ALP detector

using namespace mpeg7cdvs;

CdvsClientLowMem::CdvsClientLowMem(const CdvsConfiguration * config, int mode):CdvsClientImpl(config, mode)
{}

CdvsClientLowMem::~CdvsClientLowMem() {
}


unsigned int CdvsClientLowMem::encode(CdvsDescriptor & cdvsDescriptor, int width, int height, const unsigned char * input) const
{
	cdvsDescriptor.clear();		// erase old data

	AlpDetectorLowMem imagebuffer;
